...
通过实现移动构造函数和移动赋值操作符，设置了一个成员\textit{movedFrom}，并在输出操作符中求值。
...
对于具有可调用对象的算法，允许规范提供详细的功能，元素通过\textit{std::move()}传递给可调用对象。在可调用对象内部，可以决定如何处理:
...
\begin{shell}
coll (4 elems): "don't" "vote" "for" "liars" _
- process(don't) _
- process(for) _
- process(liars) _
coll (4 elems): "?" "vote" "?" "?"
