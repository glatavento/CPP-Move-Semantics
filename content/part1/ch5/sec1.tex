...
开始迭代之前，初始化\cppinline{reference1}，必须使用传递的范围两次（一次调用begin()，一次调用end()），并希望避免创建副本（这可能代价很高，甚至不可能）。一般来说，引用会延长所引用内容的生命周期。在本例中，\cppinline{range}并不引用由\cppinline{returnPersonByValue()}返回的\cppinline{Person}，而\cppinline{range}指向\cppinline{getName()}的返回值，是对返回的\cppinline{Person}的引用。因此，\cppinline{range}扩展了引用的生命周期。因此，在第一个语句的末尾，返回的临时对象将销毁，并且在遍历名称的字符时使用对已销毁对象名称的引用。
...
\begin{itemize}
	...
	\item 带有\cppinline{const}\&限定符的版本用于所有其他情况。总是合适的，但只是无法获得\&\&版本时的备选方案。因此，如果有一个不会销毁的对象或没标记为\cppinline{std::move()}，就会使用此函数。
\end{itemize}
...
语句1)和3)使用\cppinline{const}\&的版本，因为有一个对象没有使用\cppinline{std::move()}标记。语句2)和4)使用\&\&的版本，因为我们为一个临时对象调用\cppinline{getName()}。因为临时对象即将销毁，getter可以将成员名移出作为返回值，这意味着不必为返回值分配新的内存，可以窃取了成员的值。
...
使用\cppinline{getName()}时，使用\cppinline{std::move()}将提高程序的性能。不返回对\cppinline{const} std::string的引用（只能复制），返回值是作为非\cppinline{const}字符串返回的\cppinline{p}的移动名，因此\cppinline{push_back()}可以使用移动语义将其移动到\cppinline{coll}中。通常，在这之后，\cppinline{p}处于有效但未定义的状态。
