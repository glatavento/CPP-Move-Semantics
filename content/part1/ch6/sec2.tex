...
当\cppinline{customer}移走时，保证\cppinline{name}和\cppinline{values}都处于有效的状态，以便析构函数（由\cppinline{customer}的析构函数调用）可以正常工作:
...
另外，给\cppinline{c}赋一个新值也可以，所以就同时赋了\cppinline{name}和\cppinline{values}。
...
开始时，通过将两个任务传递给\cppinline{Tasks}对象\cppinline{ts}来启动它们。因此，在\cppinline{ts}中线程数组有两个元素，\cppinline{numThreads}为2。但是，容器的移动操作会移动元素，因此在\cppinline{std::move()}之后，\cppinline{ts}不再包含任何运行的线程对象。因此，\cppinline{numThreads}只是复制而已，也就是创建了一个无效的任务。析构函数最终会对前两个任务调用join()，这会抛出一个异常（这将导致析构函数中出现致命错误）。
