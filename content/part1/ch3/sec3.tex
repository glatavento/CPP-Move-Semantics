...
换句话说，如果移动构造函数或移动赋值操作符显式声明（使用\cppinline{=default}实现、生成，或使用\cppinline{=delete}禁用），那么通过使用\cppinline{=delete}声明，就禁用了调用复制构造函数和复制赋值操作符。
...
因为没有发现生成的特殊的移动成员函数，所以现在会复制临时的\cppinline{Customer}，甚至标记为\cppinline{std::move()}的\cppinline{Customer}对象：
