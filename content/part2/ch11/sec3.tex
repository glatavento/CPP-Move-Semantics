...
所有有效引用都延长了所绑定值的生命周期，这也适用于rvalue引用。但是，没有绑定到\cppinline{createString()}的返回值（这样可以正常工作）;而是绑定到引用，该引用指向由\cppinline{at()}返回的\cppinline{createStrings()}的返回类型，扩展了引用的生命周期。因此，该循环将遍历已经销毁的字符串。
