...
请记住，传递给rlvalue引用的rvalue在使用时成为lvalue，需要\cppinline{std::move()}再次将其作为rvalue传递。但是，有些地方不能使用\cppinline{std::move()}。对于其他重载，当传递rvalue时，使用\cppinline{std::move()}将调用\cppinline{foo()}的重载实现来获取rvalue引用。
...
否则，将调用\cppinline{foo(const X\&)}。这通常没问题的，但在某些情况下，可能希望保留传递\cppinline{const} rvalue引用的信息（例如，出于某种原因，提供了\cppinline{foo(const X\&\&)}的重载）。
