\section{总结}
\begin{itemize}
	\item 可以使用通用引用来绑定到所有 \cppinline{const} 和非 \cppinline{const} 对象，而不会丢失该对象的 \cppinline{const} 信息。
	\item 即使不使用 \cppinline{std::forward<>()}，也可以使用通用引用来实现对传递的参数的特殊处理。
	\item 要拥有特定类型的通用引用，需要使用 \cppinline{concepts/requirements}（自C++20起）或一些模板技巧（到C++17为止）。
