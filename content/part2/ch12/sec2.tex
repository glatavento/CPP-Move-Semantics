...
\begin{itemize}
	\item 因\cppinline{为std::move(s)}是一个xvalue，所以\cppinline{da5}是rvalue引用。
	\item 因为字符串的加法操作符按值返回新的临时字符串（因此它是prvalue），所以\cppinline{da6}是普通值类型。
	\item 因为\cppinline{s[0]}返回对第一个字符的lvalue引用，所以它是lvalue，并强制\cppinline{da7}也是lvalue引用。
	\item 因为\cppinline{(s)}是lvalue，所以\cppinline{da8}是lvalue引用。是的，这里会因为括号有所不同。
\end{itemize}
...
该函数将\cppinline{args}声明为一个可变数量的通用引用（也称为转发引用）。通过\cppinline{std::forward<>()}，它完美地将这些给定的参数转发给f作为第一个参数传递的函数。因为我们使用decltype(auto)作为返回类型，所以完美地将\cppinline{f()}的返回值返回给\cppinline{call()}的调用者。因此，可以同时调用按值返回和按引用返回的函数。例如：
...
函数\cppinline{call()}调用函数\cppinline{nextString()}，不带任何参数，并返回它的返回值来初始化\cppinline{s}。
...
函数调用带有\cppinline{std::move()}标记的函数\cppinline{returnArg()}。\cppinline{returnArg()}将传递的参数作为右值引用返回，然后\cppinline{call()}完美地返回给调用者来初始化ref。str仍然有它的值，并且ref会对其进行引用：
...
将\cppinline{s}和\cppinline{ref}的值移动到\cppinline{str}，得到以下状态：
...
函数使用\cppinline{std::cout}和\cppinline{ref}作为完全转发的参数调用\cppinline{print()}函数。\cppinline{print()}将传递的流作为lvalue引用返回，然后完美地返回给\cppinline{call()}的调用者。
...
\begin{itemize}
	\item 如果\cppinline{ret}声明为rvalue引用，decltype(auto)使用表达式\cppinline{std::move(ret)}，这是xvalue，来推导rvalue引用。因此，将\cppinline{f()}返回的值移动到这个函数的调用者。
...
\end{cppcode}
\cppinline{call()}的返回类型正确。但是，如果\cppinline{f()}返回rvalue引用，则不能返回左值\cppinline{ret}，因为非\cppinline{const}引用没有绑定到lvalue。
