...
据我所知，他们后来一致认为最好采用rvalue引用的方法。然而，真正的答案不重要。\textit{std::move()}的规则也应该适用于这里:如果用\textit{std::move()}传递只移动的对象，可能会丢失值，也可能不会。如果放弃所有权很重要(希望确保文件已关闭、线程已停止或相关的资源已释放)，请在调用后使用相应语句直接明示。例如:
